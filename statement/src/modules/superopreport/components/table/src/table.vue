<template>
	<div class="am_table">
		...
		<el-table
			...
			:height="wrapHeight">
			<el-table-column width="200" :fixed="index === 0 && fixed" v-for="(item, index) in columnData" :key="index" :label="item.label" :prop="item.index">
...
export default {
	...
	methods: {
		...
		sortTable(a, b, index) {
			let aNumber = parseFloat(a[index]);
			let bNumber = parseFloat(b[index]);
		...
		renderCell(row, column, cellValue, index) {
			const { property } = column;
			let currentColumnConfig = this.columns[property] || {};
			if ((cellValue === null || cellValue === '') && currentColumnConfig.kpidepend === '1' && ((this.dataType === 0 && this.needConfigs[row[0]]) || (this.dataType === 1 && this.needConfigs))) {
				const handleClick = function () {
					window.parent.location.href = `${location.origin}/shair/superOperationReport!config.action`
				}
				let firstRowEl = <el-popover
					placement="top"
					width="180"
					trigger="hover">
					<div>
						<p>该KPI需配置门店经营数据方能查看</p>
						<br />
						<el-button onClick={handleClick} type="primary" size="mini">配置</el-button>
					</div>
					<am-icon slot="reference" size="12px" name="info"></am-icon>
				</el-popover>
				return <p class="cell_content">配置{firstRowEl}</p>;
			}
			// 占比颜色区分
			let className = 'cell_content';
			let styleObj = {};
			
			let NumberVal = parseFloat(cellValue);
			let value = NumberVal;
			if (isNaN(NumberVal) || !NumberVal) {
				value = column.label === '所属门店' ? cellValue : '--';
			} else if (cellValue && (typeof cellValue === 'string') && cellValue.indexOf('%') > -1) {
				value = `${parseFloat(NumberVal.toFixed(2))}%`
			} else {
				value = parseFloat(NumberVal.toFixed(2));
			}
			...
			return <p style={styleObj} class={className}>{tipsIconName ? <am-icon size="12px" name={tipsIconName}></am-icon> : ''}{value}</p>;
		},
		...
		getTableColumnData(data) {
			...
			headTop.forEach((item, index) => {
				...
				if (colspan) {
					columnData.children = [];
					columnData.isIndeterminate = false;
		...
		init() {
			...
			if (this.dataType === 0) {
				this.tableData.forEach(item => {
					let node = this.getSearchNode(item[0]);
					if (!node) return;
					let result = this.$parent.validNodeNeedConfig(node);
					this.needConfigs[item[0]] = result;
				})
			}
			if (this.dataType === 1) {
				this.needConfigs = false;
				this.requestData.searchObj && this.requestData.searchObj.forEach(item => {
					const { id, shopId, areaId, level } = item; 
					let node = this.$parent.getSearchObjNode({
						id,
						shopId: (shopId && id && shopId !== id) ? shopId : undefined,
						areaId: level === 1 ? id : areaId
					});
					if (!node) return;
					let result = this.$parent.validNodeNeedConfig(node);
					if (result) {
						this.needConfigs = true;
					}
				})
			}
