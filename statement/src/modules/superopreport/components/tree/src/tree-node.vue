<template>
	<div 
		...
		<div class="am_tree_node-content"
			...
			<span @click.stop="handleExpandIconClick" :class="{'is-leaf': node.isLeaf || !showExpandIcon, 'expanded': expanded}" class="am_tree_node-expand_icon el-icon-caret-right"></span>
			<span
				v-if="node.loading"
			...
			<!-- <div class="am_tree_node-check_all">
				<el-checkbox
					v-if="node.childNodes && node.childNodes.length > 0 && !disabledCheckAll"
...
export default {
	...
	computed: {
		disabledCheckAll() {
			return this.tree.disabledSelectAll ? this.tree.disabledSelectAll(this.node) : false;
		},
		showExpandIcon() {
			return this.tree.showExpandIcon ? this.tree.showExpandIcon(this.node) : true;
		}
	},
	methods: {
		handleClick() {
		...
		handleExpandIconClick() {
			if (this.node.isLeaf || !this.showExpandIcon) return;
			if (this.expanded) {
				this.node.collapse();
			} else {
				this.node.expand();
	...
	created() {
		...
		if (this.node.level === 1 && this.node.data.level < 2) {
