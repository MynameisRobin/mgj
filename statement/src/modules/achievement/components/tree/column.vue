		...
		<div class="tree_column">
			<div v-if="isFormula" :class="['tree_column-value', columnData.type]" :style="isItemsScroll">
				<FormulaInput
					...
					:factorList="tree.formulaSplice"
					:pay-list="tree.payList"
					:index="index"
					@valid="handleInputValid">
					<template slot="prepend">
						<el-popover
							v-if="tree.formulaSplice[0] !== '项目原价'"
							popper-class="popInfo"
							placement="bottom"
							width="200"
							trigger="hover">
							<el-checkbox v-model="columnData.enabledPerfPct" title="自动计算欠款的占比">
								{{'自动计算欠款的占比' | cutString(20)}}
							</el-checkbox>
			...
			<div v-if=" columnData.type != 'step' && !isFormula" :class="['tree_column-value', columnData.type]" ref="container">
				...
				<div v-if="columnData.value.length > 0">
					<span v-if="tree.columnsConfig[0].type != columnData.type && copyData == null " class="fuzhi"
						@click="copyFun">
			...
			<div v-if="columnData.children && !isUpdate" :class="['tree_column-child']" >
				<div :class="{'child_content': true, 
					'packContent': ( (columnData.type == 'step' && columnData.children[0].type == 'itemids') || columnData.type == 'itemids') && tree.tabPaneType == 4 }" :style="[downClassFun(keyIndex)]">
					<tree-column
...
export default {
	...
	data() {
		return {
			...
			equationInfo: [
				{
					title: '固定业绩欠款的隐含逻辑：',
					content: '勾选“自动计算欠款的占比”，可实现固定业绩时，自动去除欠款占比的业绩'
				},
				{
					title: '举例：',
					content: '固定业绩为 50，支付金额分别为现金 200，欠款 50，则业绩计算时为 40'
				}
			]
		}
	},
	computed: {
		...
		isHidden() {
			...
			return this.tree.hiddenList.indexOf(type) >= 0;
		},
		isItems() {
		...
		isItemsScroll() {
			...
			if(this.columnData.type === 'itemids') {
				...
				let index = FindIndex(this.tree.headersWidth, item => item.type === this.columnData.type);
				if (index >= 0) {
					return `${this.tree.headersWidth[index].style}; height: ${heightNumber}px; overflow-y: auto`;
				}
			}
		},
		typeList() {
			let obj = {};
			this.tree.columnsConfig.forEach(k => {
				obj[k.type] = k.name;
			});
			return obj;
		},
...
<style lang="less" scoped>
	.formula_tree {
		> .tree_column_wrap:nth-of-type(1n) {
			background: #FAFAFA;
		}
		> .tree_column_wrap:nth-of-type(3n) {
			background: #fff;
		}
		> .tree_column_wrap {	
			.headTitle:nth-of-child(1n) {
				background: #FAFAFA;
			}
			.headTitle:nth-of-child(2n) {
				background: #fff;
			}
			padding: 20px 25px;
			> .headTitle {
