	...
	<div class="formula_input" :class="{'is-error': !valid}">
		<el-autocomplete
			...
			<template slot-scope="{ item }">
				<p style="height: 100%" title="“关键字”可帮助用户快速配置出合法的提成计算公式">{{ item.value }}</p>
			</template>
...
export default {
	...
	computed: {
		...
		restaurants() {
			return this.computeFactorList.map(item => {return {value: item}});
		},
		computedItemList() {
			let value = this.val;
		...
		typeLabel() {
			return this.factorList[0].substr(0, 2);
		}
	},
	methods: {
		getMatchLabel(label) {
		...
		querySearch(queryString, cb) {
			...
			const lastChars = currentVal ? this.getLastChars(currentVal.substr(0, index)) : '';
			const lastCharsIsOperator = lastChars ? this.isOperator(lastChars) : false;
			var restaurants = this.restaurants;
			if (lastChars === ')') {
				cb([]);
				return;
			};
			var results = (queryKey && !lastCharsIsOperator) ? restaurants.filter(this.createFilter(queryKey)) : restaurants;
			cb(results);
		},
