					...
					<el-checkbox-group v-model="checkUserListCopy"
						...
						<ul class="userUl scroll5">
							<li v-for="(user, userKey) in userList" :key="userKey">
								...
								<span class="name" :title="user.name || user.nickname">{{(user.name || user.nickname) | cutString(6) }}</span>
...
export default {
	...
	methods: {
		...
		userIsBonusPlanListFun() {
			let userIsBonusPlanList = [],
				maxNum = 0;
			if (this.planData.isBonus === 0) {
				userIsBonusPlanList = this.item.proposals.filter(item => item.isBonus === 0 && item.employeeNos !== null);
			} else {
				maxNum = 2;
				userIsBonusPlanList = this.item.proposals.filter(item => (item.isBonus === 1 || item.isBonus === true) && item.employeeNos !== null);
			}
			...
			list.forEach(item => { 
				...
				if (counts[item] > maxNum) {
					// push方案个数 大于 maxNum 个的员工
					maxList.push(item);
				}
			});
