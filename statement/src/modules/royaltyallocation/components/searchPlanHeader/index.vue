<template>
	<div :style="tab_contentStyle" class="tab_content">
...
export default {
	...
	watch: {
		...
		nowEnableStep(newV, oldV) {
			// console.log(11111111111, newV, oldV);
			// this.$emit('stepChange', newV);
		}
	},
	created() {
	...
	mounted() {	
		let planObj = JSON.parse(JSON.stringify(this.allPlanList[0]));
		...
		this.$emit('stepChange', this.nowEnableStep);
		
		this.tab_contentStyle.width = document.body.clientWidth + 'px';
	},
	methods: {
		...
		selectPlan(id) {
			...
			let index = FindIndex(this.allPlanList, item => item.id === id);

			if(index >= 0) {
				let planObj = this.allPlanList[index]
				this.nowId = planObj.id; 
				this.nowIsBonus = planObj.isBonus;
				// this.nowEnableStep = planObj.enableStep ? true : false;
				this.nowEnableStep = planObj.enableStep == 0 ? false : true;	
				this.nowStepEquation = planObj.stepEquation ? planObj.stepEquation : null;
				this.$emit('stepChange', this.nowEnableStep);
			}else{}
