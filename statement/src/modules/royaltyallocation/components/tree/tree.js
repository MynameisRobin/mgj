...
export default class Tree {
	...
	mapping(obj) {	
		...
		val.forEach( (k, v, arr)=> {
			...
			// let index = FindIndex(mappingV, item => {
			// 	item[this.typeKey[type]] == k;
			// });
			// if(type === 'itemids') {
			// 	console.log('mappingV', type, val, k, index);
			// }
			// if(index >= 0) {
			// 	mappingV[index].id = mappingV[index][this.typeKey[type]];
			// 	list.push(mappingV[index]);
			// }else{}
		})
		return list;
	}
	...
	coventChildren(obj) {
		...
		childrens.forEach(item => {
			...
			if (index >= 0) {
				...
				if (key === 'itemids') {
					let list = [];
					// console.log(222222222222, key, value);
				...
				if (level < configs.length - 1) {
					...
					// if (value[0] != '-1' && value[0] != 'T-1' && mappingV != undefined) {
					// 	let valueList = [];
					// 	currentItem.value.forEach( k => {
					// 		valueList.push(k[this.typeKey[key]]);
					// 	})
					// 	currentItem.valueKey = `,${valueList.join(',')},`;
					// } else {
					// 	currentItem.valueKey = currentVal;
					// }
