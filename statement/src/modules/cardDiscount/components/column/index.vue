...
export default {
    data() {
        return {
            ...
            apiUrl: {
				//新增/修改方案
                addUpTreatScheme: "/cardType!addUpTreatScheme.action",
                delTreatScheme: "/cardType!delTreatScheme.action",
                copyRules: "/cardType!copyRules.action"
            },
            parentShopId: "",
            editVm: {},
            shopNameStr: "",
    ...
    methods: {
        ...
        onSave() {
            ...
            this.$http.post(this.apiUrl.addUpTreatScheme, params).then(res => {
        ...
        delProposal(params) {
            this.singleLoding = true;
            this.$http.post(this.apiUrl.delTreatScheme, params).then(res => {
                this.singleLoding = false;
                let resData = res.data;
                const { code, content } = resData;
                if (code === 0) {
        ...
        onCopy(data) {
            this.$confirm("确定要复制此方案吗?", "提示", {
                ...
                .then(() => {
                    ...
                    this.singleLoding = true;
                    this.$http
                        .post(this.apiUrl.copyRules, params)
                        .then(res => {
                            this.singleLoding = false;
                            let resData = res.data;
                            const { code, content } = resData;
