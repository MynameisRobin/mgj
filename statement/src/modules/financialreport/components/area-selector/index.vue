...
export default {
	...
	computed: {
		isSerialReport() {
			return [3, 4, 6, 12, 14].indexOf(this.reportData.reportNo) >= 0;
		},
		isValidSubReport() {
			return [5, 6, 7, 8, 9].indexOf(this.reportData.reportNo) >= 0;
		},
		allNodes() {
	...
	methods: {
		...
		handleCheckChange(data, checkVal, checkAll) {
			...
			if (checkVal && !checkAll && data.level === 2) {
				if (this.isSerialReport || ((this.hasSubsidiary || data.id === data.parentShopId) && this.isValidSubReport)) {
					const {
						shopId,
						areaId
					} = data;
					this.$refs.tree.setCheckedKeys([data.id], shopId, areaId);
				}
			}
			if (checkAll) {
				...
				nodes.forEach(item => {
					result.push(this.getSearchObjItem(item));
				})
			}
		...
		getListData() {
			this.loading = true;
			this.$http.post('/superOperationReport!getAreas.action', { allInfo: 1, reportType: 1 }).then(res => {
				this.loading = false;
